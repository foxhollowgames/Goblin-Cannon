...
- **Task size:** Prefer **one coherent slice per task** (one subsystem, one bug, or one feature vertical) with **3–7 acceptance lines** — not a single giant task for the whole request unless the request is tiny. Splitting reduces merge risk and keeps each execution prompt focused.
