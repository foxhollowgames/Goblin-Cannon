...
| `godotTestFixRetries` | After a **failed** Godot test run (non-zero exit, not timeout), how many **extra** execution+test rounds to run. The next execution prompt includes the failing log tail so the agent can fix code or tests. **`0`** = a single Godot run (no fix retries). **`5`** = up to **6** Godot runs total. **`-1`** (default) = retry until tests pass or you **Stop** the pipeline. Env: **`ORCH_GODOT_TEST_FIX_RETRIES`**. **Timeouts** do not trigger retries (fix hangs or raise `godotHeadlessTimeoutMs`). |
| `executionRecoveryRetries` | When **execution** fails **before** Godot (CLI non-zero with no usable worktree changes, or exit 0 but **no git changes** while `requireExecutionGitChanges` is true), how many **extra** full execution attempts to run with the last CLI log injected. Default **`2`** ⇒ up to **3** execution invocations per task round. **`0`** = old behavior (fail immediately). Env: **`ORCH_EXECUTION_RECOVERY_RETRIES`**. |

Environment overrides: `GOBLIN_CANNON_ROOT`, `ORCH_WORKTREE_PARENT`, `GODOT_PATH`, **`ORCH_GODOT_HEADLESS_TIMEOUT_MS`**, **`ORCH_GODOT_TEST_FIX_RETRIES`**, **`ORCH_EXECUTION_RECOVERY_RETRIES`**, **`CURSOR_AGENT_BIN`** (preferred) or **`CURSOR_CLI`**, `ORCH_PORT`, `ORCH_HOST`. **`CURSOR_AGENT_BIN`** is checked **first**: use it for the standalone **`agent.cmd`** path so a user-level **`CURSOR_CLI`** pointing at **`cursor.cmd`** does not override your config.
