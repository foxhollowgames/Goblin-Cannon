...
# Fixed seed points for procedural rocks and grass tufts within a single repeat block
...
## Sets scroll speed immediately in pixels per second.
...
## Applies a brief ground shudder impulse on cannon firing.
func trigger_recoil_rumble(intensity: float = 3.0) -> void:
	...
	_rumble_tween.tween_property(self, "_rumble_offset", Vector2.ZERO, 0.2) \
		.set_trans(Tween.TRANS_ELASTIC).set_ease(Tween.EASE_OUT)
	_rumble_tween.tween_callback(func(): queue_redraw())
	queue_redraw()
#endregion
...
func _draw_roadway_base() -> void:
	...
	draw_line(Vector2(ROAD_LEFT_X, 0), Vector2(ROAD_LEFT_X, TERRAIN_HEIGHT), c_verge_border, 2.0)
	draw_line(Vector2(TERRAIN_WIDTH - ROAD_LEFT_X, 0), Vector2(TERRAIN_WIDTH - ROAD_LEFT_X, TERRAIN_HEIGHT), c_verge_border, 2.0)
...
func _draw_repeating_slices() -> void:
	...
	while start_y < TERRAIN_HEIGHT + TILE_REPEAT_Y:
		# Draw pebbles on road
		for pt in _detail_pebbles:
			draw_circle(Vector2(pt.x, start_y + pt.y), 2.5, c_pebble)
		# Draw grass tufts on verges
		for tuft in _detail_verge_tufts:
			draw_line(Vector2(tuft.x - 3, start_y + tuft.y), Vector2(tuft.x + 3, start_y + tuft.y - 4), c_grass, 1.5)
			draw_line(Vector2(tuft.x, start_y + tuft.y), Vector2(tuft.x, start_y + tuft.y - 5), c_grass, 1.5)
