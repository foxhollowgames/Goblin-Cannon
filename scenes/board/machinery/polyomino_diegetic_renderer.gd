...
static func draw_diegetic_overlays(canvas: CanvasItem, module_node: Node2D, module_data: PolyominoModuleData, components: Array, components_by_cell: Dictionary, accent_col: Color, banner_text: String, banner_timer: float, is_ghost: bool = false) -> void:
	if module_data != null and not is_ghost:
		...
		for comp_item in components:
			...
			match comp_item.cell_type:
				PolyominoModuleData.CellType.DROP_TARGET:
					var is_down: bool = (comp_item is DropTargetScript and comp_item.is_dropped)
					if is_down:
				...
				PolyominoModuleData.CellType.ROLLOVER_SWITCH:
					var is_lit: bool = (comp_item is RolloverSwitchScript and comp_item.is_lit)
					if is_lit:
				...
				PolyominoModuleData.CellType.BALL_LOCK, PolyominoModuleData.CellType.BALL_TRAP, PolyominoModuleData.CellType.SCOOP_SINKHOLE:
					var is_trapping: bool = (comp_item.get("_captured_balls") is Array and not comp_item.get("_captured_balls").is_empty())
					if is_trapping:
		...
		if module_data.goal_type == GoalArchetype.SEQUENCE_ROUTE:
			...
			if not target_seq.is_empty():
				var cur_step: Vector2i = target_seq[mini(module_node.get_current_hit_count(), target_seq.size() - 1)]
	...
	# Floating comic banner text on goal achievement
