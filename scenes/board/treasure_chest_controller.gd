...
@export var active_duration_sec: float = 120.0
## Uniform delay between chest cycles (idle): mean 120s → ~one spawn per 2 minutes on average.
@export var min_interval_sec: float = 90.0
@export var max_interval_sec: float = 150.0
@export var event_row_y: float = 312.0
...
func _start_preview(board: Node2D) -> void:
	...
	if _preview_node == null:
		var scr: GDScript = load("res://scenes/board/treasure_chest_preview.gd") as GDScript
...
func _tick_preview(delta: float, board: Node2D) -> void:
	...
	if board.has_method("spawn_treasure_chest_peg_at"):
		_active_peg_id = board.spawn_treasure_chest_peg_at(_preview_pos, event_x_min, event_x_max)
	else:
		_active_peg_id = -1
	if _active_peg_id < 0:
		_reset_to_idle()
...
func _tick_active(board: Node2D) -> void:
	...
	if now_ms >= _active_deadline_ms:
		if board.has_method("remove_treasure_chest_peg"):
			board.remove_treasure_chest_peg(_active_peg_id)
		on_treasure_event_ended(false)

func on_treasure_event_ended(_claimed: bool) -> void:
	_state = _State.IDLE
	_active_peg_id = -1
	if _preview_node:
		_preview_node.visible = false
...
## Debug: arm the next treasure chest event immediately (preview → peg). Returns false if not IDLE or not FIGHTING.
