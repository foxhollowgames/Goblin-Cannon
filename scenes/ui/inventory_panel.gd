...
func _rebuild() -> void:
	...
	sep.color = Color(0.4, 0.35, 0.5, 0.8)
	...
	if GameState:
		...
		gold_lbl.add_theme_color_override("font_color", Color(0.95, 0.82, 0.35, 1))
...
func _build_balls_section(parent: VBoxContainer) -> void:
	...
	if bag_count > 0:
		_add_body_text(parent, "  Bag (overflow)  ×%d" % bag_count, Color(0.75, 0.65, 0.45, 1))
...
func _build_upgrades_section(parent: VBoxContainer) -> void:
	...
	if not wall_upgrades.is_empty():
		...
		for uid in ids:
			...
			if stacks > 1:
				name_str += "  ×%d" % stacks
			_add_body_text(parent, "  %s" % name_str, Color(0.95, 0.75, 0.4, 1))
			var desc: String = info.get("description", "")
			if not desc.is_empty():
				_add_body_text(parent, "    %s" % desc, Color(0.7, 0.65, 0.75, 1), 12)
	...
	if not boss_upgrades.is_empty():
		...
		for uid in ids:
			var info: Dictionary = _get_upgrade_info(uid)
			var name_str: String = info.get("name", String(uid))
			_add_body_text(parent, "  %s" % name_str, Color(0.95, 0.55, 0.25, 1))
			var desc: String = info.get("description", "")
			if not desc.is_empty():
				_add_body_text(parent, "    %s" % desc, Color(0.7, 0.65, 0.75, 1), 12)
...
func _add_section_header(parent: VBoxContainer, text: String) -> void:
	...
	label.add_theme_color_override("font_color", Color(0.8, 0.7, 0.95, 1))
	parent.add_child(label)

func _add_body_text(parent: VBoxContainer, text: String, color: Color = Color(0.9, 0.9, 0.9, 1), font_size: int = 14) -> void:
	var label: Label = Label.new()
	label.text = text
...
func _add_stat_row(parent: VBoxContainer, stat_name: String, value: String) -> void:
	...
	value_label.add_theme_color_override("font_color", Color(0.95, 0.85, 0.4, 1))
